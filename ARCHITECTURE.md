...
├── notebooks/        # Prototype API (Colab) — [à ajouter avant le rendu]
...
**Hébergement pour la continuité après le rendu** : à finaliser par le
groupe. Le pipeline doit continuer à tourner en autonome au-delà de la
deadline (le cours IA1 consomme les données au fil de l'eau). Options
envisagées : VPS gratuit (Oracle Cloud Free Tier, etc.) ou machine dédiée du
groupe laissée allumée en continu.
